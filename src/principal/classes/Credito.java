...
public class Credito extends Formadepagamento {
	...
	private final float TAXA_PERCENTUAL_COBRANCA = 2;

	public Credito() {
		this.form = Forma.CREDITO;
	}
	...
	public boolean transacao(Comprador comp,Vendedor vend) {		
		...
		float valorCobradoComTaxaAoComprador = valoresapagar + (valoresapagar * (TAXA_PERCENTUAL_COBRANCA/100));
		comp.valoresapagar.add(valorCobradoComTaxaAoComprador);
		vend.valoresAReceber.add(valoresapagar);
		for (Map.Entry<Produto, Integer> entry : comp.CarrinhodeCompras.entrySet()) {
			Produto key = entry.getKey();
			Integer value = entry.getValue();
